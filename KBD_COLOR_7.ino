...
const int logoWidth = 74;  // ajuste conforme exportação
...
struct Asteroid {
  ...
  int size;
  int points;
  int angle[10]; // ângulos dos vértices
  int radius[10]; // distância de cada vértice
};
  ...
  if (idle_time > LIFE_TIMEOUT) {
    ...
    if (now - last_ss_switch > SS_SWITCH_INTERVAL) {
      ...
      switch (current_ss_type) {
        ...
        case 5: draw_atari_logo(); break;
        case 6: init_asteroids(); break;
      }
      last_ss_switch = now;
    }
    ...
    if (now - last_gen_time > speed) {
      switch (current_ss_type) {
        ...
        case 5: update_atari(); break;
        case 6: update_asteroids();break;
      }
      g_display_dirty = true;
      last_gen_time = now;
    }
...
uint16_t neonPulseColor(int pos, float intensity, int phase) {
  ...
  byte r = (sin(0.05 * (pos + color_offset) + 0) * 127 + 128) * pulse * intensity;
  byte g = (sin(0.05 * (pos + color_offset) + 2) * 127 + 128) * pulse * intensity;
  byte b = (sin(0.05 * (pos + color_offset) + 4) * 127 + 128) * pulse * intensity;
  return tft.color565(r, g, b);
}

void update_atari() {
  color_offset += 2;  // velocidade da animação
  if (color_offset > 2000) color_offset = 0;
}
...
void draw_atari_logo() {
  ...
  tft.drawString("ATARI", x0 + 10, y0 + logoHeight + 5);
}
...
/*
  void tuh_hid_report_received_cb(uint8_t d, uint8_t i, uint8_t const *r, uint16_t l) {
    ...
    } else if (l == 3 && r[0] == 0x03) {
      // Relatório de mídia (volume)
      int delta = (r[1] == 0xE9) ? 2 : (r[1] == 0xEA) ? -2 : 0;
      if (delta != 0) {
        g_volume = constrain(g_volume + delta, 0, 100);
        rp2040.fifo.push_nb(FIFO_DISPLAY_UPDATE);  // força redesenho imediato
      }

      // Repassa para o PC
      uint8_t media_data[2] = { r[1], r[2] };
      if (usb_hid.ready()) {
        usb_hid.sendReport(2, media_data, 2);
      }
    }

    // Continua ouvindo o teclado
    tuh_hid_receive_report(d, i);
  } */
